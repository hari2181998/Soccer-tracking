...
if __name__ == "__main__":
    ...
    if args.benchmark == 'MOT20':
        ...
        MOT = 17
    elif args.benchmark == "SN20":
        train_seqs = [i for i in range(60,116)]
        train_seqs1 = [i for i in range(151,175)]
        train_seqs = train_seqs.extend(train_seqs1)

        test_seqs = [ i for i in range(116, 150)]
        test_seqs1 = [i for i in range(176,200)]
        test_seqs = test_seqs.extend(test_seqs1)
        seqs_ext = ['']
        MOT = 20
    else:
    ...
    for ext in seqs_ext:
        for i in seqs:
            if args.benchmark == "SN20":
                seq = 'SNMOT' + str(MOT) +'-' + str(i)
            elif i < 10:
